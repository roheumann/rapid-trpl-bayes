...
def solve_transient(tarr, npulse, Eg, Nc, Nv, krad, Nt, Et, taun, taup, T=300, bias=0):
    ...
    # Trim to PL peak (mirrors experimental trim_and_normalize), so that
    # trap-filling artefacts before the peak don't produce negative tau.
    peak_idx = np.argmax(PL_tr)
    t      = t[peak_idx:]
    n_tr   = n_tr[peak_idx:]
    p_tr   = p_tr[peak_idx:]
    nt_tr  = nt_tr[peak_idx:]
    QFLS_tr = QFLS_tr[peak_idx:]
    PL_tr  = PL_tr[peak_idx:]
